...
@Tag(name = "企业端-信用", description = "信用档案、调整记录、争议工单")
@RestController
@RequestMapping("/api/company/credit")
...
public class CompanyCreditController {
    ...
    public ResponseEntity<ApiResponse<UserCredit>> myCredit() {
        return ResponseEntity.ok(ApiResponse.success(userCreditPortalService.myCredit(UserRole.COMPANY)));
    }
    ...
    public ResponseEntity<ApiResponse<PageResponse<CreditAdjustLog>>> adjustLogs(
        ...
        return ResponseEntity.ok(ApiResponse.success(
                userCreditPortalService.myAdjustLogs(UserRole.COMPANY, current, size)));
    }

    @Operation(summary = "争议对象候选项（来自曾投递本企业的学生）")
    @GetMapping("/dispute-counterparties")
    public ResponseEntity<ApiResponse<List<DisputeCounterpartyVO>>> disputeCounterparties() {
        return ResponseEntity.ok(ApiResponse.success(
                userCreditPortalService.listDisputeCounterparties(UserRole.COMPANY)));
    }
    ...
    public ResponseEntity<ApiResponse<List<DisputeRelationPickVO>>> disputeRelationPicks(
        ...
        return ResponseEntity.ok(ApiResponse.success(
                userCreditPortalService.listDisputeRelationPicks(UserRole.COMPANY, targetUserId, relatedType)));
    }
    ...
    public ResponseEntity<ApiResponse<PageResponse<CreditDisputeSimpleVO>>> disputes(
        ...
        return ResponseEntity.ok(ApiResponse.success(
                userCreditPortalService.myDisputes(UserRole.COMPANY, current, size, scope)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> submitDispute(@Valid @RequestBody CreateCreditDisputeRequest req) {
        userCreditPortalService.submitDispute(UserRole.COMPANY, req);
    ...
    public ResponseEntity<ApiResponse<Void>> supplement(
        ...
        userCreditPortalService.submitDisputeSupplement(UserRole.COMPANY, id, supplementText, list);
