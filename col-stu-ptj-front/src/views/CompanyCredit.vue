<template>
  <div class="credit-page credit-page--company">
    ...
    <CreditDisputeDialog
      v-model="dlg"
      :load-counterparties="getCompanyDisputeCounterparties"
      :load-relation-picks="getCompanyDisputeRelationPicks"
      :submit-dispute="submitCompanyCreditDispute"
...
import {
  getCompanyCredit,
  getCompanyCreditAdjustLogs,
  getCompanyCreditDisputes,
  getCompanyDisputeCounterparties,
  getCompanyDisputeRelationPicks,
  submitCompanyCreditDispute,
  submitCompanyDisputeSupplement
} from '../api/company'
...
const submitSupplement = async () => {
  ...
  try {
    ...
    await submitCompanyDisputeSupplement(suppRow.value.id, fd)
...
const loadProfile = async () => {
  ...
  try {
    const res = await getCompanyCredit()
...
const loadTab = async () => {
  ...
  try {
    if (tab.value === 'logs') {
      const res = await getCompanyCreditAdjustLogs(q)
      if (res.success && res.data) Object.assign(rows, res.data)
    } else {
      const res = await getCompanyCreditDisputes({ ...q, scope: disputeScope.value })
      if (res.success && res.data) Object.assign(rows, res.data)
    }
  } finally {
    loading.value = false
...
.page-hero {
  ...
  overflow: hidden;
  margin-bottom: 20px;
  background: linear-gradient(135deg, #4a3d6b 0%, #6b5b9a 45%, #8f7ec8 100%);
  color: #fff;
}
...
.page-hero-bg {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 92% 8%, rgba(255, 255, 255, 0.12), transparent 40%),
    radial-gradient(circle at 4% 92%, rgba(220, 200, 255, 0.2), transparent 48%);
  pointer-events: none;
}
